...
- **双协议对外**:同时暴露 OpenAI 兼容 `/v1/chat/completions` 与 Anthropic 兼容 `/v1/messages`(均含 SSE 流式)。
- **配置从 JSON 文件读取**。
...
| `model_map` | object | `{}` | 外部 model 名 → KSCC 实际 model 名映射。命中则替换,未命中透传。 |
...
│   └── convert_openai.py       #   OpenAI↔Anthropic 格式转换 + 流事件→SSE 映射
├── config/                     # 配置文件目录
...
```
别的客户端 ──(OpenAI/Anthropic 协议)──> 本中转(localhost:8787)
                                          │
                          ┌───────────────┴───────────────┐
                          │                               │
                  OpenAI 端点:格式转换          Anthropic 端点:字节透传
                          │                               │
                          └───────────┬───────────────────┘
