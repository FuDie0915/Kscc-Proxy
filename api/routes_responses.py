...
async def responses(request: Request) -> Any:
    ...
    if req.stream:
        try:
            ...
            ms = int((time.perf_counter() - t0) * 1000)
            logger.warning("%s %s 502 %dms model=%s create_stream failed: %s", method, path, ms, model, exc)
    ...
    try:
        ...
        ms = int((time.perf_counter() - t0) * 1000)
        logger.warning("%s %s 502 %dms model=%s create failed: %s", method, path, ms, model, exc)
        return _error_response(exc)
    ms = int((time.perf_counter() - t0) * 1000)
